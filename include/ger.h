...
#include <cblas.h>
   ...
   template <> int GER<float>(int_t m, int_t n, float alpha, float *x, int_t incx, float *y, int_t incy, float *A, int_t ldA)
   {
      if(m<0)
         return -1;
      else if(n<0)
   ...
   template <> int GER<double>(int_t m, int_t n, double alpha, double *x, int_t incx, double *y, int_t incy, double *A, int_t ldA)
   {
      if(m<0)
         return -1;
      else if(n<0)
